...
Mil (thousandth of an inch) is commonly used to express barcode density in the imperial measurement system. This is convenient, firstly because the values are expressed as whole numbers, and secondly because barcode density can be more easily related to the resolution of printing devices, which is usually expressed in dots per inch (dpi). The dimensions of a barcode are very important for successful scanning. Each barcode type is defined using the following size parameters:
...
This is a very important attribute, especially for linear barcodes. Spacing is the light areas at the start and the end of the barcode. They are required for the scanner to identify the barcode borders. It's much better if these areas will be the same color as the barcode background. For most two-dimensional matrix symbologies, the requirements for the presence of free zones are less stringent or absent. Most modern scanners contain more powerful controllers and "intelligent" decoding algorithms to read barcodes printed without blank areas.
