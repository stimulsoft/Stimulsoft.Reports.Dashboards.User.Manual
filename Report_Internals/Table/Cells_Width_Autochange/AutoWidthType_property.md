...
Column width is set depending on the table width. In other words the width of all column cells is checked first (the column width is set by the longest line). If there is free space then it is equally distributed between all columns. If there is not enough space to output the longest lines, then the width of columns is decreased in equal parts between all columns.
...
Column width is set depending on the table width. In other words the width of all column cells is checked first (the column width is set by the longest line). If there is free space then it is distributed to the last column whose **FixedWidth** property is set to **false**. If there is not enough space to output the longest lines, then the width of the last columns is decreased and distributed between all columns whose **FixedWidth** properties are set to **false**.
