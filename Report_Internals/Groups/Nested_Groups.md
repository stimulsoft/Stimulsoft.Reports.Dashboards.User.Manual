...
* **Note:** It is recommended to have an equal number of GroupHeader and GroupFooter bands in a report.


In each **Group Header** band you must specify the grouping criteria. When rendering the report the **Group Header** bands are processed in the order in which they appear on a page working from the top down, the topmost band is processed first, then the one that is placed directly underneath it and so on. When placing **Group Footer** bands on a report page it is important to remember that the last **Group Footer** band is always associated with the first **Group Header** band.
