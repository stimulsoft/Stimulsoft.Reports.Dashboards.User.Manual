...
| start Designer.exe -data=d:\MyData | To launch the report designer with binding to XML data sources. The value for the data parameter is the path to the folder containing XML data files. |
| start Designer.exe d:\Report.mrt -run | To launch the report viewer with a report template *.mrt, *.mrz, or *.mrx, or a generated report *.mdc, *.mdz, or *.mdx. |
| start Designer.exe -schedule scheduleName | To launch the scheduler where the scheduler's name is passed as its value. All created schedulers can be found in the path "c:\Users\%USERNAME%\AppData\Local\Stimulsoft\Scheduler". |
