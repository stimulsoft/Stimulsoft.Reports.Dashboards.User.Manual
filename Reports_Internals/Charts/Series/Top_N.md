...
As can be seen from the picture, a kind of filtering is performed, i.e., Report Generator has identified five minimum values, and the rest grouped into a single value. Because the property found in the Show Other value lies (false), then grouped the value does not appear on this chart.
